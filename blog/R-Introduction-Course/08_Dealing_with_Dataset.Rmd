...
Often it is useful to learn how to create a dataset in R. Above we created one, which we will reproduce the code here. A dataset can be created in R with the function data.frame(), and we fill it with the variables we would like to create. We conviniently named our dataset _data_. Note that after we specified the function data.frame(), we can give each of the variables its own name. After that, we assign to that name our wanted values.

We use set.seed() because it is a function in R that allows all 'pseudo-random' fuctions in R to yield the same results in different computers. It has the purpose of reproducibility. Otherwise, all of you would have different values for the pseudo-random values. Cool, right?
...
Or alternatevely, we can also print the last 4 rows to see whether we created the data well.

```{r}
tail(data, 4)
```

Importantly, you can explore your data in a new window with the command View(). Note that there are two buttons on the left hand side: one which will pop-up the data in a new window, and the other that you can use to filter the cases you are interested in specting.
...
R is telling you where specifically - out of all your variables - the error is occuring , and also, its reason: "_arguments imply differing number of rows: 9, 10_". In my eyes, it even gives you the solution: There are nine rows, but we need 10 to make it work. 

### 12.2 Indexing [or Data Manipulation]
...
Another method is to uset the subset() function in R. The subset function is available in base R and can be used to return subsets of a vector, martix, or data frame which meet a particular condition.
...
## 12.3 Indexing vectors

Remember we created three vectors above, _n.R.courses_, _height_ and _height.female_. Lets use these to learn how to index vectors.
...
If I would like to select my first and my second observations of _n.R.courses_, I make use of the ':' opperator, which denotes sequence. So, for R, I am effectively telling it I want observations from 1 to 2.
...
What if I would like to know which are the observed heights are taller than 170 cm? In this case we can make use of logical opperators. As reviewed above, we saw that logical operations return either TRUE or FALSE. So, we can use this as to 'select' the observations we want given a condition. So we just write it as above:
...
## 10.4 Understanding your data

Let's assume we received this data from someone, and we have to analyize it. One of the first thing you need to do is to _understand_ the basic characteristics of your data. How many variables/collums does it have? How many observations? What are the characteristics of each variable? And so on. 

### Dimensions

Here you are asking R to return the number of rows and collumns.
...
Here you are asking R to return the characteristics of each variable in your dataset. It shows a summary of each variable, and the first observations. For example, it tells us that there are _10 obs. of  6 variables_. It also itemizes each variable by name, and displays the class of each variable. Gender, for example, is a factor with two levels, one for Male and another for Female. Levels are the different possible varieties of a factor. We could mae out of Height one factor with levels "tall" and "short". Names, for its turn has 10 levels because it only present unique entries (e.g., the different names for each one of our hypothesized participants)
